...
export default async function ({login, q, imports, data, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    for (const {type, limit} of [
      {type: "issueComments", limit: _limit1},
      {type: "issues", limit: _limit2},
      {type: "repositoryDiscussionComments", limit: _limit3},
      {type: "repositoryDiscussions", limit: _limit4},
    ].filter(({limit}) => limit)) {
      let cursor = null,
        pushed = 0
      const fetched = []
      try {
        do {
          ...
          } = await graphql(queries.reactions({login, type, before: cursor ? `before: "${cursor}"` : ""}))
          ...
          const filtered = edges
            .flatMap(
              ...
              }) => ({created: new Date(created), reactions: reactions.filter(({user = {}}) => imports.filters.text(user.login, ignored)).map(({content}) => content)}),
            )
    ...
    const total = Object.values(list)
      .map(({value}) => value)
      .reduce((a, b) => a + b, 0)
