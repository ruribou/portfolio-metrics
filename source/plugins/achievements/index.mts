...
function rank(x, [c, b, a, s, m]) {
  ...
  if (x >= a) return {rank: "A", progress: (x - a) / (s - a)}
  else if (x >= b) return {rank: "B", progress: (x - b) / (a - b)}
  else if (x >= c) return {rank: "C", progress: (x - c) / (b - c)}
              ...
              const results =
                (await page.evaluate(() => [
                  ...([...document.querySelectorAll("h2")]
                    .filter(node => /Filter by/.test(node.innerText))
                    .shift()
                    ?.nextSibling?.innerText.trim()
                    .matchAll(/(?<type>Repositories|Users|Issues)\n.*?(?<count>\d+)M/g) ?? []),
                ])) ?? null
