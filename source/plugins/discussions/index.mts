  ...
  try {
    ...
    //Load started discussions
    {
      ...
      do {
        ...
        const {
          user: {repositoryDiscussions: {edges = [], nodes = []} = {}},
        } = await graphql(queries.discussions.categories({login, after: cursor ? `after: "${cursor}"` : ""}))
    ...
    //Load comments
    {
      ...
      do {
        ...
        const {
          user: {repositoryDiscussionComments: {edges = [], nodes = []} = {}},
        } = await graphql(queries.discussions.comments({login, after: cursor ? `after: "${cursor}"` : ""}))
        cursor = edges?.[edges?.length - 1]?.cursor
        fetched.push(...nodes)
        pushed = nodes.length
