...
export default async function ({login, q, imports, data, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    const {
      user: {repository},
    } = await graphql(queries.licenses.repository({owner: data.repo.owner.login, name: data.repo.name, account}))
    ...
    if (await imports.which("licensed")) {
      ...
      if (
        ...
        await imports.fs.writeFile(imports.paths.join(path, ".licensed.yml"), ["cache_path: .licensed"].join("\n"))
      } else {
      ...
      JSON.parse(await imports.run("licensed list --format=json --licenses", {cwd: path})).apps.map(({sources}) =>
    ...
    const list = Object.entries(used)
      .map(([key, count]) => ({name: licenses[key]?.spdxId ?? `${key.charAt(0).toLocaleUpperCase()}${key.substring(1)}`, key, count, value: count / total, x: 0, color: licenses[key]?.color ?? "#6e7681", order: licenses[key]?.order ?? -1}))
      .sort((a, b) => (a.order === b.order ? b.count - a.count : b.order - a.order))
