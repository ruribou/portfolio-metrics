...
;(metadata as any).plugin = async function ({__plugins, __templates, name, logger}) {
  try {
    ...
    //Web metadata
    {
      meta.web = Object.fromEntries(
        Object.entries(inputs as Record<string, any>)
          .map(([key, {type, description: text, example, default: defaulted, min = 0, max = 9999, values, extras}]) => [
            //Format key
    ...
    //Readme metadata
    {
      ...
      const demo = meta.examples
        ? demos({examples: meta.examples})
        : (raw
      ...
      const header = [
        ...
        `    <td>${Object.entries(compatibility)
          .filter(([_, value]) => value)
          .map(([id]) => `<a href="/source/templates/${id}/README.md"><code>${templates[id].name ?? ""}</code></a>`)
          .join(" ")}</td>`,
        "  </tr>",
        "  <tr>",
        `    <td>${[meta.supports?.includes("user") ? "<code>👤 Users</code>" : "", meta.supports?.includes("organization") ? "<code>👥 Organizations</code>" : "", meta.supports?.includes("repository") ? "<code>📓 Repositories</code>" : ""].filter(v => v).join(" ")}</td>`,
        "  </tr>",
        "  <tr>",
        `    <td>${
          [
            ...
            ...Object.entries(inputs as Record<string, any>)
              .filter(([_, {type}]) => type === "token")
              .map(([token]) => `<code>🗝️ ${token}</code>`),
            ...
            .filter(v => v)
            .join(" ") || "<i>No tokens are required for this plugin</i>"
        }</td>`,
        "  </tr>",
        "  <tr>",
...
;(metadata as any).template = async function ({__templates, name, plugins}) {
  try {
    ...
    const header = [
      ...
      `    <td>${Object.entries(compatibility)
        .filter(([_, value]) => value)
        .map(([id]) => `<a href="/source/plugins/${id}/README.md" title="${plugins[id].name}">${plugins[id].icon}</a>`)
        .join(" ")}${meta.formats?.includes("markdown") ? " <code>✓ embed()</code>" : ""}</td>`,
      "  </tr>",
      "  <tr>",
      `    <td>${[meta.supports?.includes("user") ? "<code>👤 Users</code>" : "", meta.supports?.includes("organization") ? "<code>👥 Organizations</code>" : "", meta.supports?.includes("repository") ? "<code>📓 Repositories</code>" : ""].filter(v => v).join(" ")}</td>`,
      "  </tr>",
        ...
        compatibility: {
          ...
          ...Object.fromEntries(
            Object.entries(compatibility)
              .filter(([_, value]) => !value)
              .map(([key, value]) => [key, meta.formats?.includes("markdown") ? "embed" : value]),
          ),
          base: true,
        },
        header,
...
function demos({colspan = null, wrap = false, examples = {}} = {}) {
  if ("default1" in examples && "default2" in examples) {
    return [wrap ? '<td colspan="2"><table><tr>' : "", '<td align="center">', `<img src="${examples.default1}" alt=""></img>`, "</td>", '<td align="center">', `<img src="${examples.default2}" alt=""></img>`, "</td>", wrap ? "</tr></table></td>" : ""].filter(v => v).join("\n")
  }
  return [
