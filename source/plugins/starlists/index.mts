...
export default async function ({login, q, imports, data, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    _languages_aliases = Object.fromEntries(
      _languages_aliases
        .split(",")
    ...
    for (const list of lists) {
              ...
              language: {
                ...
                color:
                  element
                    ...
                    ?.style?.backgroundColor?.match(/\d+/g)
      ...
      if (languages) {
        ...
        list.languages = Object.entries(list.languages)
          .filter(([name]) => !_languages_ignored.includes(name.toLocaleLowerCase()))
          .sort((a, b) => b[1] - a[1])
          .slice(0, _limit_languages || Infinity)
