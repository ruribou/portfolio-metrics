  ...
  try {
    ...
    for (const {repository, owner} of repositories) {
      ...
      do {
        ...
        } = await graphql(queries.stargazers({login: owner, repository, after: cursor ? `after: "${cursor}"` : "", location: _worldmap ? "node { location }" : ""}))
    ...
    const increments = {
      dates: Object.fromEntries([
        ...new Array(days)
          .fill(null)
          .map((_, i) => [new Date(Date.now() - i * 24 * 60 * 60 * 1000).toISOString().slice(0, 10), 0])
          .reverse(),
      ]),
      max: NaN,
      min: NaN,
    ...
    if (["graph", "chartist"].includes(_charts_type) && imports.metadata.plugins.stargazers.extras("charts.type", {extras})) {
      ...
      charts = await Promise.all(
        [
          {data: total, low: total.min, high: total.max},
          {data: increments, low: 0, high: increments.max, sign: true},
        ].map(({data: {dates: set}, low, high, sign = false}) =>
          imports.Graph.timeline(
            Object.entries(set).map(([x, y]) => ({x: new Date(x), y, text: imports.format(y, {sign})})),
            {low, high, match: (data, ticks) => data.filter(([x]) => ticks.map(t => t.toISOString().slice(0, 10)).includes(x.toISOString().slice(0, 10)))},
          ),
        ),
      )
    }
    ...
    if (_worldmap && imports.metadata.plugins.stargazers.extras("worldmap", {extras})) {
      const {default: generate} = await import("./worldmap/index.mts")
