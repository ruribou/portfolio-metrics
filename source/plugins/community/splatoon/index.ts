...
export default async function ({login, q, imports, data, account}, {enabled = false, extras = false, token, "statink.token": _statink_token} = {}) {
  ...
  try {
    ...
    //Read fetched data
    const exported = await Promise.all((await imports.fs.readdir(`${imports.__module(import.meta.url)}/s3si/${source === "mocks" ? "mocks" : "export"}`)).map(async file => JSON.parse(await imports.fs.readFile(`${imports.__module(import.meta.url)}/s3si/${source === "mocks" ? "mocks" : "export"}/${file}`))))
    ...
    const player = {
      ...
      plate: {
        color: `#${Math.round(255 * summary.data.HistoryRecordQuery.currentPlayer.nameplate.background.textColor.r)
          .toString(16)
          .padStart(2, 0)}${Math.round(255 * summary.data.HistoryRecordQuery.currentPlayer.nameplate.background.textColor.g)
          .toString(16)
          .padStart(2, 0)}${Math.round(255 * summary.data.HistoryRecordQuery.currentPlayer.nameplate.background.textColor.b)
          .toString(16)
          .padStart(2, 0)}`,
      ...
      equipment: {
        ...
        gears: await Promise.all(
          ["headGear", "clothingGear", "shoesGear"].map(async type => {
          ...
          fetched
            ...
            .map(async ({data}) => ({
              ...
              teams: await Promise.all(
                [data.detail.myTeam, ...data.detail.otherTeams].map(async team => ({
                  color: `#${Math.round(255 * team.color.r)
                    .toString(16)
                    .padStart(2, 0)}${Math.round(255 * team.color.g)
                    .toString(16)
                    .padStart(2, 0)}${Math.round(255 * team.color.b)
                    .toString(16)
                    .padStart(2, 0)}`,
                  score: (data.detail.vsRule.name === "Turf War" ? team.result?.paintRatio * 100 : team.result?.score) ?? null,
                  players: await Promise.all(
                    team.players.map(async ({name, byname, weapon, paint, result, isMyself: self}) => ({
                      name,
                      byname,
                      self,
                      weapon: {
    ...
    if (sections.includes("salmon-run")) {
      salmon = {
        matches: await Promise.all(
          fetched
            .filter(({type}) => type === "COOP")
            .slice(0, _salmon_limit)
            .map(async ({data}) => ({
