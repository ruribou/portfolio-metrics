...
export default async function ({login, q, imports, data, graphql, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    const {
      [account]: {
        sponsorsListing: {fullDescription, activeGoal},
      },
    } = await graphql(queries.sponsors.description({login, account}))
    ...
    let list = []
    {
      ...
      do {
        ...
        const {
          [account]: {
            sponsorshipsAsMaintainer: {edges, nodes},
          },
        } = await graphql(queries.sponsors.active({login, account, after: cursor ? `after: "${cursor}"` : "", size: Math.round(size * 1.5)}))
    ...
    if (past) {
      ...
      const users = []
      {
        ...
        do {
          ...
          const {
            [account]: {
              sponsorsActivities: {edges, nodes},
            },
          } = await graphql(queries.sponsors.all({login, account, after: cursor ? `after: "${cursor}"` : "", size: Math.round(size * 1.5)}))
          cursor = edges?.[edges?.length - 1]?.cursor
          fetched.push(...nodes)
          pushed = nodes.length
