...
export default async function ({login, q, imports, data, rest, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    try {
      for (let page = 1; page <= pages; page++) {
        ...
        events.push(
          ...[
            ...(await Promise.all([
              ...(context.mode === "repository" ? await rest.activity.listRepoEvents({owner: context.owner, repo: context.repo}) : await rest.activity.listEventsForAuthenticatedUser({username: login, per_page: 100, page})).data
    ...
    if (snippet) {
      ...
      const indent = Math.min(...(snippet.patch.match(/^.\s+/gm) ?? [0]).map(line => line.length - 1)) || 0
      const content = imports.htmlescape(snippet.patch.replace(new RegExp(`^(.)[\\t ]{0,${indent}}`, "mg"), "$1"))

      //Format patch
      snippet.patch = imports
        .htmlunescape(await imports.highlight(content, "diff"))
        .trim()
        .replace(/\r?\n/g, "<br/>")
