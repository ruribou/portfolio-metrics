...
export default async function ({login, data, imports, q, rest, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    aliases = Object.fromEntries(
      aliases
        .split(",")
        .filter(alias => /^[\s\S]+:[\s\S]+$/.test(alias))
        .map(alias => alias.trim().split(":"))
    ...
    for (const {section, stats = {}, lines = {}, missed = {bytes: 0}, total = 0} of [
      {section: "favorites", stats: languages.stats, lines: languages.lines, total: languages.total, missed: languages.missed},
      {section: "recent", ...languages["stats.recent"]},
    ]) {
      ...
      languages[section] = Object.entries(stats)
        ...
        .slice(0, limit)
        .map(([name, value]) => ({name, value, size: value, color: languages.colors[name], x: 0}))
        .filter(({value}) => value / total > threshold)
      if (other) {
        let value = indepth
          ? missed.bytes
          : Object.entries(stats)
              .filter(
                ([name]) =>
                  !Object.values(languages[section])
                    .map(({name}) => name)
                    .includes(name),
              )
              .reduce((a, [_, b]) => a + b, 0)
      ...
      const visible = {
        total: Object.values(languages[section])
          .map(({size}) => size)
          .reduce((a, b) => a + b, 0),
      }
      for (let i = 0; i < languages[section].length; i++) {
        ...
        if (colors[i] && !colors[name.toLocaleLowerCase()]) languages[section][i].color = colors[i]
        else languages[section][i].color = customColors[name] ?? languages.colors[name] ?? fallbackColors[name.toLocaleLowerCase()] ?? "#ededed"
      }
    }
